...
type TrainingClientProps = {
  initialTraining?: TrainingCert[] | null
  initialProfile?: AuthMeResponse | null
}

export function TrainingClient({ initialTraining, initialProfile }: TrainingClientProps) {
  const { locale } = useLocale()
  const { t } = useTranslation()
  const { persistLocale, isSaving: isSavingLocale } = usePersistedLocale()

  const [page, setPage] = useState(1)
  const [trainingRows, setTrainingRows] = useState<TrainingCert[]>(() => initialTraining ?? [])
  ...
  const {
    ...
    mutate: refreshCerts,
  } = useApiData<TrainingCert[]>("/training", undefined, {
    refreshInterval: 180_000,
    fallbackData: initialTraining ?? undefined,
  })
  ...
  }, [certs, profile, refreshCerts])

  useEffect(() => {
    if (certs) {
      setTrainingRows([...certs])
    }
  }, [certs])

  useEffect(() => {
    if (certsError && !(certsError instanceof ApiError && certsError.status === 401)) {
      toast.error(t("toasts.loadError"))
    }
  }, [certsError, t])

  const statusFor = useCallback((record: TrainingCert) => {
  ...
  const handleRenew = useCallback(
    (record: TrainingCert) => {
      toast.info(t("training.renewReminder"), {
        description: record.worker_name,
      })
      setMockToastMessage(t("training.renewReminder"))
    },
    [t],
  )

  const columns = useMemo<ColumnDef<TrainingCert>[]>(() => {
    return [
      {
        header: t("training.table.worker"),
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">{record.authority ?? ""}</p>
            </div>
          )
        },
      },
      {
        header: t("training.table.certification"),
        accessorKey: "certification_type",
        cell: ({ row }) => <span className="text-sm text-slate-600">{row.original.certification_type}</span>,
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="space-y-1 text-sm text-slate-600">
              ...
              <Badge variant={badgeVariant[statusCode]}>{statusLabel(statusCode)}</Badge>
            </div>
          )
        },
      },
      {
        header: t("training.table.status"),
        id: "status",
        cell: ({ row }) => {
          ...
          return <Badge variant={badgeVariant[statusCode]}>{statusLabel(statusCode)}</Badge>
        },
      },
      {
  ...
  const pageCount = Math.max(1, Math.ceil(trainingRows.length / PAGE_SIZE))
  const currentPage = Math.min(page, pageCount)
  const paginated = trainingRows.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE)
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <DataTable<TrainingCert>
          key={locale}
          columns={columns}
          data={paginated}
          isLoading={certsLoading && trainingRows.length === 0}
          loadingMessage={t("app.loading")}
          emptyMessage={t("empty.training")}
          pageSize={PAGE_SIZE}
        />
        <div className="mt-6">
          <Pagination
